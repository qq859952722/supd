...
**必填字段**：`name`、`version`、`command`、`runtime`
...
```yaml
run_as: root   # 或 service / <用户名>
```

- 热重载，下次执行生效
- `root`：以 root 身份运行（需 supd 有权限）
- `service`：以所属服务的 user/group 运行（服务级扩展默认，DEV-005 部分接入）
- `<用户名>`：以指定用户身份运行（需 supd 有权限解析）
- 省略：以 supd 自身身份运行
