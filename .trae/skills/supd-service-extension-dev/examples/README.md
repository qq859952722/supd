...
- 服务和扩展示例目录都必须包含中文 `README.md`，并包含各自规范要求的全部精确一级标题，且不得包含敏感数据
- 服务或扩展的行为、协议、配置契约或部署逻辑发生变更时，必须同步对应 `README.md`，并在“变更记录”中追加日期和摘要
- `service.yaml` 的 `name` 字段必须与所在目录名完全一致
- `meta.yaml` 的 `entry` 为相对扩展自身目录的路径（如 `run.sh` / `run.js`），也支持绝对路径；不可带冗余 `./` 前缀
