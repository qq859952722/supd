...
- `service.yaml` 保存服务配置，由管理员或 supd 管理。
...
此示例未定义必需环境变量。实际部署时在 `env.yaml` 的 `env:` 下配置非敏感变量。若需要敏感变量，只记录变量名称和用途，例如 `API_TOKEN` 用于访问外部 API，不写入其值。

# 服务级扩展与 Actions

此示例没有服务级扩展和 actions。需要时可在 `extensions/` 下添加符合 supd 规范的扩展。

# 数据持久化与升级更新

示例不持久化业务数据。实际服务应将可保留配置和状态置于 `data/`；升级时替换 `bin/` 中的版本化载荷，保留数据并在失败时回滚到先前版本。
