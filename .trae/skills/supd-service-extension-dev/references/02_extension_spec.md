...
每个全局或服务级扩展目录**必须**包含中文 `README.md`，用于让后续开发者不阅读全部脚本即可理解扩展用途、执行边界和维护入口。新建扩展时创建；修改 action、触发器、运行逻辑、外部接口、协议、下载来源、环境变量或部署要求时同步更新。

至少包含以下一级标题：

1. `# 扩展概述`：名称、当前版本、全局/服务级范围、一句话用途和适用场景。
2. `# 触发方式与 Actions`：列出 trigger、action、并发策略、超时、运行身份和副作用。
3. `# 运行逻辑`：按步骤说明 `SUPD_ACTION` 分支、输入、处理、输出、临时文件及失败/回滚路径；复杂流程可用 Mermaid。
4. `# 配置与环境变量`：说明自定义变量及使用到的 `SUPD_*` 变量，不写敏感值。
5. `# 开发与外部资源`：记录上游仓库/API、官方下载 URL 或生成规则、版本和架构映射、校验方式；涉及协议或格式转换时记录协议版本、关键字段、单位/编码、转换规则及简短示例。没有外部依赖时明确写“无”。
6. `# 部署与特别注意事项`：记录 runtime、依赖工具、架构/libc、权限、CWD/路径基准、网络和已验证的部署坑点。
7. `# 验证与故障排查`：给出校验、手动触发、结果查询、日志定位及典型错误识别方法。
8. `# 变更记录`：按 `YYYY-MM-DD | 版本（可选） | 修改内容 | 影响/验证` 倒序记录有意义的修改。

README 应维护当前有效结论，不堆积已完成待办或冗长尝试过程；失效内容直接修正或删除。下载链接优先使用官方稳定入口/生成规则，不记录临时签名 URL。不得包含真实密码、token、私钥、Cookie、内网敏感地址或运行期敏感数据。
...
- **消息内双引号转义（仅 `::progress::`/`::result::`）**：消息内容中的双引号需转义为 `\"`（如 `::result:: success "say \"hello\""`）。`::notify::` 的 content **所见即所得、不做转义还原**，请勿写入 `\"`（会按字面保留）。
