...
- **核心文档**：`docs/需求规格说明_v1.6.md`（业务规则唯一权威来源）
...
- `docs/需求规格说明_v1.6.md` — 业务规则（涉及业务逻辑时按章节读取，是唯一权威来源）
...
| 错误码 | 22个（见§5.4） | §5.4 |
| 通知等级 | info/success/warning/error（4种） | 设计稿 §六.1 |
...
| 文件历史版本 | 50个 | §2.3.1 |
| 服务阶段并发上限 | 4 | 设计稿 §四.3 |
| Topic 保留 | 30天/单Topic 500条 | 设计稿 §七.6 |
| 操作执行历史 | 30天/200条 | 设计稿 §0(v5) |
| notify 协议行长 | 8KB | 设计稿 §六.1 |
| runResults 容量 | 500 | 设计稿 §Phase 0.4 |

### 3.3 架构约束

- **引入 SQLite 仅限通知中心/操作中心持久化**（`modernc.org/sqlite` 纯 Go，数据库位于 `<baseDir>/data/supd.db`），**不开放脚本访问**；仍禁止其他数据库（Bolt/Badger 等）与 SSE/WebSocket
- **禁止引入SSE/WebSocket**（长轮询是规格要求）
- **禁止添加需求规格说明书中不存在的配置字段**
- **业务逻辑有疑问时，以 `docs/需求规格说明_v1.6.md` 为唯一权威来源**
...
2. **对照规格**：如涉及业务逻辑，查阅 `docs/需求规格说明_v1.6.md` 相关章节
...
- **代码逻辑变更必须同步 Skill 文档**：每次项目代码逻辑（行为/契约/接口/路径语义/生命周期等）发生变更，必须主动核对并同步 `.trae/skills/supd-service-extension-dev/` 下相关 Skill 文档，保持源码与 Skill 描述一致。仅有纯文档/注释/格式调整时无需同步。
- **Skill 远程运维操作不写入 docs**：通过 Skill 执行的远程服务运维操作（如远程 SSH 调试、远程服务目录调整、远程扩展验证等）**不需要**记录到 `docs/devlog/`。仅当运维操作揭示了项目代码缺陷并触发了实际代码修复时，才将代码修复部分按第六节流程归档。
...
*supd 项目协议 — 维护阶段。业务规则唯一权威来源：`docs/需求规格说明_v1.6.md`*
