...
func (b *Bootstrap) startService(ctx context.Context, name string, svcEntry *watch.ServiceEntry) (*Process, *logging.ServiceLogger, *RestartEngine, context.CancelFunc, error) {
	...
	proc, err := StartServiceProcess(name, command, env, workdir, svcConfig.User, svcEntry.ConfigPath, extraFiles...)
	if err != nil {
		// 进程启动失败，转移至 failed
		if preChecker != nil {
			preChecker.Close()
...
func (b *Bootstrap) superviseService(ctx context.Context, name string, svcEntry *watch.ServiceEntry, sm *StateMachine, proc *Process, engine *RestartEngine) {
	...
	// REQ-F-023, §2.2.13: 通过 StartServiceProcess 解析 user 字段，
	// user 不存在或非 root 切换其他用户时返回 *ServiceError 拒绝启动
	newProc, err := StartServiceProcess(name, command, env, workdir, svcEntry.Config.User, svcEntry.ConfigPath, extraFiles...)
	if err != nil {
		if preChecker != nil {
			preChecker.Close()
		}
