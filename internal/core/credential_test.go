...
func TestResolveServiceCredential_EmptyUser(t *testing.T) {
	cred, err := ResolveServiceCredential("test-svc", "", "/etc/supd/services/test-svc/service.yaml")
	if err != nil {
...
func TestResolveServiceCredential_NonexistentUser(t *testing.T) {
	...
	cred, err := ResolveServiceCredential("test-svc", fakeUser, "/etc/supd/services/test-svc/service.yaml")
	if err == nil {
	...
	if !strings.Contains(msg, "adduser") || !strings.Contains(msg, "useradd") {
		t.Errorf("error message should suggest adduser/useradd, got: %s", msg)
	}
...
func TestResolveServiceCredential_CurrentUser(t *testing.T) {
	...
	cred, err := ResolveServiceCredential("test-svc", curUsername, "/etc/supd/services/test-svc/service.yaml")
	if err != nil {
...
func TestResolveServiceCredential_RootUser(t *testing.T) {
	cred, err := ResolveServiceCredential("test-svc", "root", "/etc/supd/services/test-svc/service.yaml")
	curUID := uint32(os.Getuid())
...
func TestResolveServiceCredential_EmptyConfigPath(t *testing.T) {
	const fakeUser = "nonexistent_user_xyz_12345"
	_, err := ResolveServiceCredential("test-svc", fakeUser, "")
	if err == nil {
		t.Fatal("expected error for nonexistent user, got nil")
	}
	var se *svcerr.ServiceError
