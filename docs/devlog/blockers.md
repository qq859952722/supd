...
3. **M-04-001**（-0.160）：superviseService 圈复杂度 43（gocyclo 实测，架构偏差，修复需重构，违反 AGENTS.md "禁止趁机重构"原则）

> 注：原 **M-03-001**（yaml v4 rc）已重新定性为**有意技术决策**（见需求规格说明 v1.5），从技术债清单移除，不再计入扣分。
